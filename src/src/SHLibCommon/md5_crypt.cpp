...
**
** Project       : OCTOPUS 
** File Name     : cpptemplate.cpp
** Description   : SIM系统C++实现文件模板
** Contact       : zhaming@cn.is-one.net
...

{
	...
	for(i = 0; i < 5; i++) {
		...
		__data->crypt_3_buf[i + 2] = (char)(bin_to_ascii((v1 >> shf) & 0x3f));
	}
	...
	for(i = 5; i < 10; i++) {
		shf = (56 - 6 * i);
		__data->crypt_3_buf[i + 2] = (char)(bin_to_ascii((v2 >> shf) & 0x3f));
	}
	
	__data->crypt_3_buf[12] = (char)bin_to_ascii(s);
