...
#ifndef ACE_ASYNCH_CONNECTOR2_C 
#define ACE_ASYNCH_CONNECTOR2_C 

#include "ace/Asynch_Connector2.h"
...
ACE_Asynch_Connector2<HANDLER>::ACE_Asynch_Connector2 (void)
: default_addr_   ()
, monitor_        ()
, flg_cancel_     (false)
, pending_count_  (0)
...
ACE_Asynch_Connector2<HANDLER>::get_pending_count (void) const
{
...
template <class HANDLER> int
ACE_Asynch_Connector2<HANDLER>::open (int pass_addresses,
                                     ACE_Proactor *proactor,
                                     int validate_new_connection)
{
    ACE_TRACE ("ACE_Asynch_Connector2<>::open");
...
template <class HANDLER> int
ACE_Asynch_Connector2<HANDLER>::connect (
                        ...
                        const void *act)
{
                                     ...
                                     act) == -1)
    {
        ACE_ERROR_RETURN ((LM_ERROR,
                       ACE_LIB_TEXT ("%N:%l %p\n"),
                       ACE_LIB_TEXT ("ACE_Asynch_Connector2::connect")),
                      -1);
    }
...
template <class HANDLER>
const typename ACE_Asynch_Connector2<HANDLER>::ADDRESS&
ACE_Asynch_Connector2<HANDLER>::get_completion_address (
...
ACE_Asynch_Connector2<HANDLER>::handle_connect (
                const ACE_Asynch_Connect::Result &result)
{
       ...
       ACE::clr_flags (result.connect_handle (), ACE_NONBLOCK) != 0)
    {
       ...
       ACE_ERROR ((LM_ERROR,
          ACE_LIB_TEXT ("%p\n"),
          ACE_LIB_TEXT ("ACE_Asynch_Connector2::handle_connect: set blocking mode")));
    }
    ...
    if (!error)
    {
        ...
        if (new_handler == 0)
        {
          error = 1;
          ACE_ERROR ((LM_ERROR,
             ACE_LIB_TEXT ("ACE_Asynch_Connector2::handle_connect: make_handler failed")));
        }
    }
...
template <class HANDLER> int
ACE_Asynch_Connector2<HANDLER>::validate_connection
            (const ACE_Asynch_Connect::Result&  /*result*/,
...
template <class HANDLER> void
ACE_Asynch_Connector2<HANDLER>::wait (void)
{
    Guard_Monitor guard (monitor_);
...
template <class HANDLER> int
ACE_Asynch_Connector2<HANDLER>::cancel (void)
{
    ACE_TRACE ("ACE_Asynch_Connector2<>::cancel");
...
ACE_Asynch_Connector2<HANDLER>::make_handler (const void * /*act*/)
{
    return make_handler();
}

template <class HANDLER> HANDLER *
ACE_Asynch_Connector2<HANDLER>::make_handler (void)
{
    // Default behavior
    HANDLER *handler = 0;
...
template <class HANDLER> int
ACE_Asynch_Connector2<HANDLER>::pass_addresses (void) const
{
    return this->pass_addresses_;
}

template <class HANDLER> void
ACE_Asynch_Connector2<HANDLER>::pass_addresses (int new_value)
{
    this->pass_addresses_ = new_value;
}

template <class HANDLER> int
ACE_Asynch_Connector2<HANDLER>::validate_new_connection (void) const
{
    return this->validate_new_connection_;
}

template <class HANDLER> void
ACE_Asynch_Connector2<HANDLER>::validate_new_connection (int new_value)
{
