 ...
 *****************************************************************
 */
ACE_POSIX_AIO_Dev_Poll_Provider::~ACE_POSIX_AIO_Dev_Poll_Provider()
{
    fini ();
}
...
int 
ACE_POSIX_AIO_Dev_Poll_Provider::get_specific_flags () const
{
    // return 0;
...
int 
ACE_POSIX_AIO_Dev_Poll_Provider::init ()
{
...
int 
ACE_POSIX_AIO_Dev_Poll_Provider::fini ()
{
    if (this->fd_poller_ != ACE_INVALID_HANDLE)
    {
...
int
ACE_POSIX_AIO_Dev_Poll_Provider::interrupt ()
{
...
// *****************************************************************
int
ACE_POSIX_AIO_Dev_Poll_Provider::declare_interest (ACE_POSIX_AIO_FD_Info * info,
                                               int interest,
                                               int remain_mask)
{
...
int
ACE_POSIX_AIO_Dev_Poll_Provider::clear_interest (ACE_POSIX_AIO_FD_Info * info)
{
...
int
ACE_POSIX_AIO_Dev_Poll_Provider::poll_ready_fd (Guard_Monitor &  leader_guard,
                                           int & ready_mask,
                                           int & remain_mask)
{
...
int
ACE_POSIX_AIO_Dev_Poll_Provider::get_ready_fd (ACE_Time_Value * wait_time,
...
ACE_HANDLE
ACE_POSIX_AIO_Dev_Poll_Provider::find_next_ready_fd (int & ready_mask,
                                                     int & remain_mask)
{
    while (this->cur_index_ < this->num_remain_)
    {
        ...
        //{
        //    ACE_SET_BITS (remain_mask, ACE_POSIX_AIO_FD_Info::READ);
        //}
        //
        //if (ACE_BIT_ENABLED(pfd->events, POLLOUT))
        //{
        //    ACE_SET_BITS (remain_mask, ACE_POSIX_AIO_FD_Info::WRITE);
        //}
...
int 
ACE_POSIX_AIO_Dev_Poll_Provider::modify_poll(int begin, int end)
{
    int len = end - begin;
    if (len <= 0)
    {
    ...
    if (rc != len )
    {
        ACE_ERROR_RETURN ((LM_ERROR,
            ACE_LIB_TEXT("ACE_POSIX_AIO_Dev_Poll_Strategy::modify poll ")
...
int
ACE_POSIX_AIO_Dev_Poll_Provider::wait_for_ready_fd (ACE_Time_Value * wait_time)
{
    ACE_TRACE ("ACE_POSIX_AIO_Dev_Poll_Provider::wait_for_ready_fd");

    ACE_POSIX_AIO_FD_Info  * fd_info = 0;
