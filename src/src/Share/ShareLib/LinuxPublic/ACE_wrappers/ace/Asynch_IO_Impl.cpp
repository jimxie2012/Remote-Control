...
ACE_Asynch_Result_Impl::ACE_Asynch_Result_Impl (ACE_Allocator *allocator,
                                                void          *area)
...
void
ACE_Asynch_Result_Impl::release (ACE_Asynch_Result_Impl *result)
{
  if (result != 0)
    result->free();
}
...
int
ACE_Asynch_Result_Impl::dispatch ()
{
  int rc = -1;
  ...
  ACE_SEH_FINALLY
    {
      ...
      ACE_Asynch_Result_Impl::release (this);
    }
  return rc;
}

int
ACE_Asynch_Result_Impl::start ()
{
  ACE_Asynch_Result & original = this->get_original_result ();

  // clear FLG_FAKE_RESULT flags
  original.clr_op_flags (ACE_Asynch_Result::FLG_FAKE_RESULT);
...
int
ACE_Asynch_Result_Impl::post ()
{
  ACE_Asynch_Result & original = this->get_original_result ();

  // set FLG_FAKE_RESULT flags
  original.set_op_flags (ACE_Asynch_Result::FLG_FAKE_RESULT);
...
int
ACE_Asynch_Result_Impl::build_read_iovec (
...
int
ACE_Asynch_Result_Impl::build_write_iovec (
                   const ACE_Message_Block *msg,
                   bool                     flg_chain,
                   iovec                   *iov,
                   int &                    iovmax,
...
int
ACE_Asynch_Result_Impl::copy_iovec (
                  const iovec             *src,
                  int                      src_cnt,
                  iovec                   *iov,
                  int &                    iovmax,
