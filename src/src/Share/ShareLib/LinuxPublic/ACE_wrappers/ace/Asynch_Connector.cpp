...
template <class HANDLER> int
ACE_Asynch_Connector<HANDLER>::connect (const ADDRESS& remote_sap,
                                        ...
                                        const void *act)
{
  ...
  if (this->asynch_connect_.connect (ACE_INVALID_HANDLE,
    ...
    ACE_ERROR_RETURN ((LM_ERROR,
                       ACE_LIB_TEXT ("%N:%l %p\n"),
                       ACE_LIB_TEXT ("ACE_Asynch_Connector::connect")),
                      -1);
  return 0;
}

template <class HANDLER>
const typename ACE_Asynch_Connector<HANDLER>::ADDRESS&
ACE_Asynch_Connector<HANDLER>::get_completion_address (
...
ACE_Asynch_Connector<HANDLER>::handle_connect (const ACE_Asynch_Connect::Result &result)
{
    ...
    if (!error)
    {
        ...
        if (new_handler == 0)
        {
            error = 1;
            ACE_ERROR ((LM_ERROR,
                ACE_LIB_TEXT ("ACE_Asynch_Connector::handle_connect : make_handler failed")));
        }
    }
...
ACE_Asynch_Connector<HANDLER>::make_handler (const void * /*act*/)
{
  return make_handler();
}
